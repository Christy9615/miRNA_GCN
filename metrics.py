...
def model_evaluate(real_score, predict_score):
    return get_metrics(np.matrix(real_score),np.matrix(predict_score))

def plot_curve(x,y):
    plt.plot(x, y, linewidth = '0.1', label = "ROC", color='#0000FF', linestyle=':', marker='|')
    plt.xlabel(u"假正例率")
    plt.ylabel(u"真正例率")
    # plt.show()
...
def get_metrics(real_score, predict_score):
    ...
    plot_curve(x_ROC.A.T[0], y_ROC.A.T[0])
